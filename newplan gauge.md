...
> [!IMPORTANT]
> **Mục đích:** Phân tách rõ ràng giữa dữ liệu thô mô tả thị trường (khách quan) và các diễn giải/tín hiệu hành động dành cho nhà đầu tư.

Giao diện sẽ được chia làm **2 Tab (Thẻ)** chính:

1.  **Tab 1: Sức mạnh thị trường (Market Strength):** Hiển thị trạng thái hiện tại của thị trường dựa trên các chỉ báo. Gồm 3 đồng hồ gauge và bảng chi tiết, cho biết thị trường đang hành xử theo hướng nào (Mua/Bán/Trung tính). Dữ liệu hoàn toàn khách quan, **không phải khuyến nghị đầu tư**.
2.  **Tab 2: Tín hiệu kỹ thuật (Technical Signals):** Cung cấp các diễn giải, đánh giá và khuyến nghị (nếu có) dựa trên số liệu phân tích kỹ thuật để hỗ trợ nhà đầu tư ra quyết định (Ví dụ: Cảnh báo điểm mua/bán, quá mua/quá bán, kháng cự/hỗ trợ).
...
1.  **Vị trí hiển thị:** Thêm một nút chức năng vào **sidebar menu bên trái**. Cụ thể là nằm dưới phần **KỸ THUẬT** của nhóm **CÔNG CỤ NÂNG CAO** (cùng vị trí với các nút *Xu hướng, Key Level, Mô hình nến, Biểu đồ RRG...*). 
2.  **Cấu trúc Modal/Panel:** Khi click vào nút, một panel sẽ mở ra, bên trên cùng là **2 nút Tab** để chuyển đổi qua lại:
    - `[Sức mạnh thị trường]` (Tab 1)
    - `[Tín hiệu kỹ thuật]` (Tab 2)
3.  **Timeframe mặc định:** **1 ngày (1D)**.
4.  **Cơ chế làm mới (Refresh):** **Không** tự động làm mới (Auto-refresh) vì tính chất timeframe 1D. Cung cấp một nút **"🔄 Làm mới"** thủ công cho người dùng.

---

## 🛠️ CHI TIẾT TAB 1: SỨC MẠNH THỊ TRƯỜNG

*(Phần này sử dụng lại toàn bộ logic Đồng hồ Gauge và Bảng phân loại đã chốt từ trước)*
...
## 📈 CHI TIẾT TAB 2: TÍN HIỆU KỸ THUẬT (Dành Cho Nhà Đầu Tư)

> [!IMPORTANT]
> **Khác biệt cốt lõi:** Nếu Tab 1 đo lường *quán tính* (Momentum - giá trị cao = tiền vào mạnh = MUA), thì Tab 2 đo lường *tín hiệu giao dịch* theo phân tích kỹ thuật kinh điển (Giá trị cao = Quá mua = rủi ro đảo chiều = BÁN).

**Giao diện:** Tương tự Tab 1, gồm 3 đồng hồ Gauge (Dao động, Tổng hợp, TB Động) và các bảng chi tiết.
**Dữ liệu:** Dùng chung kết quả tính toán OHLCV từ Tab 1, nhưng **đảo ngược bộ lọc logic** cho các chỉ báo quá mua/quá bán (Oscillators).

### Logic Tín Hiệu Tab 2 (Kinh Điển) so với Tab 1

| Chỉ báo | Tab 1: Sức mạnh TT (Quán tính) | Tab 2: Tín hiệu KT (Kinh điển) | Ghi chú Tab 2 |
|---------|--------------------------------|--------------------------------|---------------|
| **RSI** | RSI > 70 → MUA | RSI < 30 → **MUA** | Dưới 30 là vùng quá bán (oversold), báo hiệu mua vào. Trên 70 là quá mua (overbought) → BÁN. |
| **STOCHK** | K > 80 → MUA | K < 20 → **MUA** | (Giống RSI) |
| **STOCHRSI**| K > 80 → MUA | K < 20 → **MUA** | (Giống RSI) |
| **CCI** | CCI > 100 → MUA | CCI < -100 → **MUA** | Kênh quá bán (-100) là tín hiệu mua. |
| **WPR** | WPR > -20 → MUA | WPR < -80 → **MUA** | Vùng quá bán sâu là tín hiệu mua. |
| **ULTOSC** | U > 70 → MUA | U < 30 → **MUA** | (Giống RSI) |
| **MACD** | MACD > Signal → MUA | MACD > Signal → **MUA** | (Giữ nguyên) Cắt lên là tín hiệu mua. |
| **MACD HIST** | > 0 & tăng → MUA | > 0 & tăng → **MUA** | (Giữ nguyên) |
| **ADX** | > 25 & giá tăng → MUA | > 25 & giá tăng → **MUA**| (Giữ nguyên) |
| **ROC** | > 0 & tăng → MUA | > 0 & tăng → **MUA** | (Giữ nguyên) |
| **SAR** | Giá > SAR → MUA | Giá > SAR → **MUA** | (Giữ nguyên) Giá nằm trên SAR là xu hướng tăng. |
| **BB WIDTH** | BBW tăng & giá tăng → MUA | BBW tăng & giá tăng → **MUA** | (Giữ nguyên) Dải BB mở rộng kèm giá tăng là MUA. |
| **TB Động** | Giá > MA → MUA | Giá > MA → **MUA** | (Giữ nguyên) Giá nằm trên MA là xu hướng tăng. |

### Cách Tính Gauge cho Tab 2
Sử dụng chính xác **4 Bước tính toán Gauge** như ở Tab 1, nhưng đầu vào (`sell_count`, `buy_count`) được lấy từ bảng phân loại logic kinh điển của Tab 2.

**Cảnh báo ở cuối Tab 2:**
> *"Các tín hiệu kỹ thuật trên dựa vào phân tích quá mua/quá bán kinh điển. Quý khách vui lòng kết hợp nhiều yếu tố khác trước khi ra quyết định giải ngân."*
